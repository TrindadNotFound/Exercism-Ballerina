...
service / on new http:Listener(9090) {
    ...
    resource function post calc(@http:Payload Calculation calculation) returns Response|http:BadRequest {
        ...
        match calculation.operator {
            ...
            "-" => { result = calculation.operand1 - calculation.operand2; }
            "x" => { result = calculation.operand1 * calculation.operand2; }
            "*" => { result = calculation.operand1 * calculation.operand2; }
            "/" => { result = calculation.operand1 / calculation.operand2; }
            _ => {
                // return <http:BadRequest>{body: "Bad operator"};
                result = 0;
                }
        }
        // Convert the two operands and the expression into a string representation with no whitespace.
        string expression = calculation.operand1.toString() + calculation.operator + calculation.operand2.toString();
